    ...
    try:
        ...
        while True:
            ...
            if frame is None:
                ...
                faces = face_detector(gray_3_channel, rgb=False)
                current_time = time.time()
                elapsed_time = current_time - start_time

                # Face alignment
                start_time = current_time
                landmarks, scores, fan_features = landmark_detector(gray_3_channel, faces, rgb=False, return_features=True)
                current_time = time.time()
                ...
                table2.append([frame_number, emo, ar, val])
